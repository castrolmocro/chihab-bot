...
const COMMANDS_INFO = {
  ستيفان:   { icon: "👼", desc: "رسائل تلقائية دورية للغروبات",        usage: "!ستيفان [رسالة] [min-max ثانية] / off / status" },
  divel:    { icon: "🌀", desc: "رسائل دورية مع انتظار عشوائي",        usage: "!divel [رسالة] [min-max] / off / status" },
  كنيات:    { icon: "✍️", desc: "تغيير كنية جميع الأعضاء باستمرار",   usage: "!كنيات [اسم] / off / status / حدف" },
  اسم:      { icon: "🔒", desc: "قفل اسم الغروب",                       usage: "!اسم [اسم] / off / time [min] [max] / status" },
  ...
  info:     { icon: "⏱️", desc: "وقت تشغيل البوت مع الإحصائيات",      usage: "!info" },
...
function buildHelpAll(prefix) {
  ...
  lines.push("╠════════════════════════════════════╣");
  lines.push(`║  ${prefix}help [اسم الأمر] للتفاصيل      ║`);
  lines.push("╚════════════════════════════════════╝");
  return lines.join("\n");
}
...
module.exports = {
  config: {
    name: "help", aliases: ["h","مساعدة","أوامر"], version: "2.0", author: "Stefan",
